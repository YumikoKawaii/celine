...
2. Recall context: filtered vector search over this client's indexed memory (owner_sub) +
   recent transcript from Postgres; enqueue an index job for the incoming user message
...
| `remember` / `recall` | Persistent long-term memory            | early     |
...
├── buf.gen.yaml                  # codegen: Go handlers + TS client
├── gen/                          # generated code (Go side; checked in or build-step)
│   └── celine/v1/...
...
│   │   └── google.go
│   ├── store/                    # Postgres (durable) + Redis (sessions)
│   │   ├── postgres.go
│   │   └── redis.go
...
Each job: embed the message text (Voyage) → insert into `memory_index` with `owner_sub`. Recall (top-K filtered vector search, §11.2) happens at the *start of the next turn* (agent loop step 2, §3.2).
...
Celine's character is **the system prompt**, but structured as three composable layers, never one prose blob. This keeps the "soul" stable and code-reviewed while letting behaviour flex per client, per project, and **per mood on a schedule** — without rewriting prose.

```
base persona (immutable prose)   ← the soul, version-controlled
        ▲ rendered with
   knobs (config dials)           ← global defaults → per-client / per-project overrides
        ▲ overridden by
   mood (scheduled overlay)       ← transient: nudges the knobs + adds a flavor line
        ↓
...
A small struct rendered *into* the prose, so behaviour tunes without prompt surgery:
...
Resolution order (later wins): **global defaults → per-project → per-client (`preferences`) → mood overlay**. Pair with config-level `temperature` (lower = steadier persona) and `model`.
...
| Per-project persona | `projects.system_prompt` + project knobs | per project |
| Per-client tuning | `clients.preferences` (new) | per client |
